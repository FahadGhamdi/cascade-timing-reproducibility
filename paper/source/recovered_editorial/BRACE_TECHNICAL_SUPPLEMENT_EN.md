# Technical supplement: What Does Timing Add to Size? Probabilistic Forecasts of Recorded Cascade Growth on Two Archives
...
## S10. Editorial clarification after consultant review

The manuscript now proposes, without claiming experimental identification, that sentinel-coded timing inputs may act as a regime-like representation in Hurdle's linear predictors. The empty-prefix state is already determined by the baseline prefix count. Consequently, this proposed mechanism concerns how an estimator represents that state; it supplies no new local event-time information. No indicator-only refit, coefficient intervention, attribution experiment or additional score calculation was performed.

The wording of the title, abstract and methods was revised without changing target definitions, feature values, forecasts, saved metrics, bootstrap intervals, eligibility rules or original protocols. Internal phase labels are retained here and in the evidence files for provenance, and removed from the main manuscript. Zhou et al. (2021) was already cited in Section 2.2 and Reference 24 before this editorial pass; no duplicate reference was added.
