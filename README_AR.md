...
## حالة الإصدار بعد فحص Mac

اكتملت إعادة التدريب154 مهمة على جلستين، والاختبار40 مهمة، والضجيج42 مهمة. راجع VALIDATION_AR.md وحدود التحقق المبينة فيه. لا تعِد التشغيل الحالي؛ الأوامر أدناه لإعادة إنتاج مستقلة مستقبلًا، بمسارات إخراج جديدة. أدلة التشغيل متاحة في validation/mac_completed.
...
اكتمل الآن التشغيل على Mac بالبيئة المطابقة، وطابقت تقارير الاختيار والنماذج المختارة الأصل وفق الأدلة المرفقة. التنفيذ استخدم87 نتيجة معتمدة و67 مهمة مستكملة؛ التقييم يستخدم الآباء التاريخية مع جسر تطابق النماذج. تفاصيل التحقق المستقل وحدوده في VALIDATION_AR.md. إعادة تغليف تشغيل جديد ستختلف بصمتها غالبًا بسبب السجلات والأزمنة حتى لو تطابقت نتائجه العلمية.
...
راجع VALIDATION_AR.md لمعرفة ما اختُبر فعليًا، وCODEX_MAC_TASK_AR.md لحالة المهمة بعد اكتمالها. يلزم Python مع NumPy/SciPy/Matplotlib للتحليلات؛ لا يتم تثبيت شيء آليًا. يتضمن environment/requirements.lock.txt الإصدارات الأصلية.
